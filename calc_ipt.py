...
CMD = "./sim_allreduce --model 1 --latency 0 --reduce-only --iterations 1000 --topology {topo} --procs {procs} --radix 2 --spread-mode {dist} --spread-avg {expected}"
OUTFILE = "reduce_{tree}_tree_{procs}_procs_{dist}_distribution_{expected}_expected_ipt"
...
def calc_ipt(num_procs, tree, is_gaussian_distribution, expected):
...
if __name__ == "__main__":
    ...
    for expected in sys.argv[2:]:
        for tree in TREES.keys():
            calc_ipt(num_procs, tree, False, int(expected))
            calc_ipt(num_procs, tree, True,  int(expected))
